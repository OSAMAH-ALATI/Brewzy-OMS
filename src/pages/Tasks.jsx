...
export default function Tasks({ onNavigate }) {
  ...
  if (session?.role !== 'manager') {
    return (
      <div className="ph">
        <div className="ph-text"><h1>Tasks</h1><p>Managers only.</p></div>
      </div>
    )
  }
  ...
  const deleteGlobal = async (i) => {
    ...
    const ok = await showConfirm({
      title: 'Delete task?',
      message: `"${t?.name || 'this task'}" will be permanently deleted from the library and removed from all machines it's assigned to. This cannot be undone.`,
  ...
  const removeMachineTask = (i) => {
    ...
    showToast('Task removed from machine')
  }
  ...
  const addToMachine = () => {
    ...
    setAddId('')
    showToast(`Task added to ${machine.name}`)
  }

  const freqTag = (freq) => <span className={`freq-tag ${FREQ_CLASS[freq] || 'freq-daily'}`}>{FREQ_LABEL[freq] || freq}</span>

  return (
    <div>
      <div className="ph">
        <div className="ph-text"><h1>Tasks</h1><p>Manage the task library and per-machine assignments</p></div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div className="card" style={{ flex: '0 0 380px', maxWidth: '100%' }}>
          <div className="card-header">
            <div className="card-title">All Tasks <span style={{ color: 'var(--text-tertiary)', fontWeight: 400 }}>· {globalTasks.length} task{globalTasks.length !== 1 ? 's' : ''}</span></div>
            <button className="btn btn-teal btn-sm" onClick={() => setEditIndex(null)}>+ Add Task</button>
          </div>
          {!globalTasks.length && (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 13 }}>No tasks yet. Click + Add Task.</div>
          )}
          {globalTasks.map((t, i) => (
            <div key={t.id || i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 0', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                <button className="btn btn-icon btn-sm" onClick={() => moveGlobal(i, -1)} disabled={i === 0} title="Move up">↑</button>
                <button className="btn btn-icon btn-sm" onClick={() => moveGlobal(i, 1)} disabled={i === globalTasks.length - 1} title="Move down">↓</button>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={() => setEditIndex(i)} title="Edit" style={{ color: 'var(--brand)' }}>✏</button>
              <button className="btn btn-ghost btn-sm" onClick={() => deleteGlobal(i)} title="Delete" style={{ color: 'var(--red)' }}>🗑</button>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ flex: '1 1 420px', minWidth: 320 }}>
          <div className="card-header">
            <div className="card-title">Machine Tasks</div>
          </div>
          ...
          {!machine && (
            <div style={{ padding: 24, color: 'var(--text-tertiary)', fontSize: 13, textAlign: 'center' }}>Select a machine to manage its assigned tasks</div>
          )}

          {machine && (
            <>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', margin: '4px 0 8px' }}>
                {machine.name} — {machineTasks.length} task{machineTasks.length !== 1 ? 's' : ''}
              </div>
              {!machineTasks.length && (
                <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 13 }}>No tasks assigned. Add from the global list below.</div>
              )}
              {machineTasks.map((t, i) => (
                <div key={t.id || i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                    <button className="btn btn-icon btn-sm" onClick={() => moveMachineTask(i, -1)} disabled={i === 0} title="Move up">↑</button>
                    <button className="btn btn-icon btn-sm" onClick={() => moveMachineTask(i, 1)} disabled={i === machineTasks.length - 1} title="Move down">↓</button>
                  </div>
                  <div style={{ flex: 1, minWidth: 0, fontSize: 13, color: 'var(--text-primary)', fontWeight: t.freq === 'tech' ? 600 : 400 }}>{t.name}</div>
                  {freqTag(t.freq)}
                  <button className="btn btn-ghost btn-sm" onClick={() => removeMachineTask(i)} title="Remove from this machine" style={{ color: 'var(--red)', flexShrink: 0 }}>✕</button>
                </div>
              ))}

              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14, marginTop: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '.7px', marginBottom: 10 }}>Add from Global Tasks</div>
                {unassigned.length ? (
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button className="btn btn-teal" onClick={addToMachine} disabled={!addId}>Add</button>
                  </div>
                ) : (
                  <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>All global tasks are already assigned to this machine.</span>
                )}
              </div>
            </>
          )}
